...
+ (void)openSafariDeeplink:(NSString *)urlString terminateFirst:(BOOL)terminateFirst {
    ...
    [self openFromSafari:urlString];
    XCTAssertTrue([app waitForState:XCUIApplicationStateRunningForeground timeout:15]);
    
    XCUIElement *testButton = app.buttons[@"Advertising"];
    XCTAssertTrue([testButton waitForExistenceWithTimeout:60 + 1]);
    [testButton tap];
}
...
+ (void)openFromSafari:(NSString *)urlString {
    ...
    [safari launch];
    XCTAssertTrue([safari waitForState:XCUIApplicationStateRunningForeground timeout:15]);
    ...
    if (buttonsQuery.count > 0) {
        XCUIElement *keyboardTutorialButton = [buttonsQuery elementBoundByIndex:0];
    ...
    XCTAssertTrue([goButton waitForExistenceWithTimeout:15]);
    [goButton tap];
    
    XCUIElement *confirmationButton = safari.buttons[@"Open"];
    XCTAssertTrue([confirmationButton waitForExistenceWithTimeout:15]);
...
+ (void)openMessagesUniversalLink:(NSString *)urlString terminateFirst:(BOOL)terminateFirst {
    ...
    [self openFromMessages:urlString];
    XCTAssertTrue([app waitForState:XCUIApplicationStateRunningForeground timeout:15]);
...
+ (void)openFromMessages:(NSString *)urlString {
    ...
    [messages launch];
    XCTAssertTrue([messages waitForState:XCUIApplicationStateRunningForeground timeout:15]);
    ...
    XCTAssertTrue([chat waitForExistenceWithTimeout:15]);
    [chat tap];
    ...
    XCTAssertTrue([bubble waitForExistenceWithTimeout:15]);
    sleep(5);
    [bubble tap];
}
